...
async function _setupSentry(id, dbUrl, port){
    ...
    const cmd = `DB_MONGO_NAME=${id} DB_MONGO_URL=${dbUrl || ''} PORT=${port} nohup node /Users/Samparsky/Sites/nodejs/adex-validator-stack-test/stack/bin/sentry.js --adapter=dummy --dummyIdentity=${id}`
    console.log({cmd})
...
async function _setupWorker(id, dbUrl){
    const cmd = `DB_MONGO_NAME=${id} DB_MONGO_URL=${dbUrl || ''} nohup node /Users/Samparsky/Sites/nodejs/adex-validator-stack-test/stack/bin/validatorWorker.js --adapter=dummy --dummyIdentity=${id} >> /dev/null &`
    const rSpawn = await spawn(cmd, {
        stdio: 'inherit',
        shell: true,
        detached: true
    })
